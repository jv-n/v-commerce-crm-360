...
export function ContactExpandedRow({ contact }: { contact: Contact }) {
  ...
  const lastPurchaseTime = pedidos.length > 0 ? parseDate(pedidos[pedidos.length - 1].data_pedido) : 0
  if (showMore) {
    timeline.push({ type: "more" as const, count: hiddenCount, time: lastPurchaseTime - 1 })
  }
  
  timeline.push({ type: "created" as const, time: -1 }) // Created is always last
